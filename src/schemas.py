...
class ContextFingerprint(BaseModel):
    ...
    # Fix: suppress model_used / model_* namespace warning from pydantic
    ...
    # GRC compliance metadata — set by context_generator in GRC mode
...
class ExecutionFingerprint(BaseModel):
    ...
    def dict_for_llm(self, include_evidence: bool = True, detail_level: str = "balanced") -> Dict[str, Any]:
        ...
        if detail_level == "summary":
            if "metrics" in result and "stage_metrics" in result["metrics"]:
                result["metrics"]["stage_metrics"] = []
            if "metrics" in result and result["metrics"].get("anomalies"):
                result["metrics"]["anomalies"] = result["metrics"]["anomalies"][:3]
...
class ProblemType(str, Enum):
    ...
    HEALTHY           = "healthy"            # ← ADDED: clean execution, no issues
    PERFORMANCE       = "performance"        # Slow queries, resource issues, optimization
    EXECUTION_FAILURE = "execution_failure"  # ← ADDED: dominant task failures
    MEMORY_PRESSURE   = "memory_pressure"    # ← ADDED: dominant spill/OOM
    SHUFFLE_OVERHEAD  = "shuffle_overhead"   # ← ADDED: dominant shuffle cost
    DATA_SKEW         = "data_skew"          # ← ADDED: dominant partition skew
    LINEAGE           = "lineage"            # Data flow, transformations, query understanding
    GENERAL           = "general"            # General questions, mixed concerns
...
class AnalysisResult(BaseModel):
    ...
    # Raw agent responses for debugging
    raw_agent_responses: Dict[str, Any] = Field(
        default_factory=dict, description="Raw responses from each agent"
    )
