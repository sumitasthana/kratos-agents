...
def _safe_first(lst: Any, default: Any = None) -> Any:
    """Safely get first element of a list, returning default if empty or None."""
...
class SemanticFingerprintGenerator:
    ...
    def _extract_dag(self) -> ExecutionDAG:
        ...
        for event in self.index.get_by_type("SparkListenerStageCompleted"):
            ...
            if stage_id is not None:
                ...
                # FIX 1: use _safe_first so empty RDD Info [] doesn't crash
                rdd_info   = stage_info_dict.get("RDD Info") or []
                first_rdd  = _safe_first(rdd_info, {})

                metadata = {
                    "num_partitions": stage_info_dict.get("Number of Tasks", 0),
                    "is_shuffle":     self._is_shuffle_stage(stage_info_dict),
                    "rdd_name":       first_rdd.get("Name"),
                    "stage_name":     stage_info_dict.get("Stage Name", f"Stage {stage_id}"),
                }
                dag.add_node(stage_id, metadata)
        ...
        # Add edges (dependencies)
        for stage_id, stage_dict in stage_info.items():
            parent_stages = stage_dict.get("Parent IDs") or []
            for parent_id in parent_stages:
                reason      = self._infer_dependency_reason(
                    stage_info.get(parent_id, {}), stage_dict
                )
                has_shuffle = self._is_shuffle_stage(stage_dict)
                dag.add_edge(
                    parent_id,
    ...
    def _compute_logical_plan_hash(
        ...
        if is_sql and physical_plan:
            ...
            plan_hash = (
                dag.hash_structure()
                if hasattr(dag, "hash_structure")
                else hashlib.sha256(
                    self._dag_structure_string(dag).encode()
                ).hexdigest()
            )
    ...
    def _is_shuffle_stage(self, stage_info: Dict[str, Any]) -> bool:
        """Determine if stage involves shuffle."""
        rdd_info = stage_info.get("RDD Info") or []
        for rdd in rdd_info:
            if "shuffle" in rdd.get("Name", "").lower():
                return True
        return len(stage_info.get("Parent IDs") or []) > 0

    def _infer_dependency_reason(
        self, parent_stage: Dict[str, Any], child_stage: Dict[str, Any]
    ) -> str:
        """Infer reason for dependency based on RDD info."""
        # FIX 2: guard both parent and child RDD Info against empty list
        parent_rdd = parent_stage.get("RDD Info") or []
        child_rdd  = child_stage.get("RDD Info") or []

        if parent_rdd and child_rdd:
            parent_name = _safe_first(parent_rdd, {}).get("Name", "")
            if "shuffle" in parent_name.lower():
                return "shuffle_dependency"
    ...
    def _describe_stage(self, stage_info: Dict[str, Any]) -> str:
        ...
        # FIX 3: safe first on RDD Info
        rdd_info = stage_info.get("RDD Info") or []
        rdd_name = _safe_first(rdd_info, {}).get("Name", "Unknown")
